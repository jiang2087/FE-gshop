...
export const recentlyViewed = ({cartKey} : {cartKey: string}) => {
      ...
      <section className="overflow-hidden py-20 bg-gray-2">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex flex-wrap items-center justify-between gap-5 mb-7.5">
            <h2 className="font-medium text-dark text-2xl">Recently Viewed</h2>
            <button onClick={handleClearWishlist} className="text-blue">Clear Recently Viewed</button>
          </div>

          {status === "loading" && (
            <div className="text-center py-5">Loading your recently viewed items...</div>
          )}
          {status === "failed" && (
            <div className="text-center py-5 text-red">Failed to load recently viewed items.</div>
          )}
          {status === "succeeded" && wishlistItems.length === 0 && (
            <div className="text-center py-5">Your recently viewed items list is empty.</div>
          )}
