...
const SingleItem = ({ item }: { item: any }) => {
  ...
  const handleAddToCart = () => {
    const defaultVariant = item.productVariants?.[0];
    if (!defaultVariant) {
      ...
      <div className="relative overflow-hidden rounded-lg bg-[#F6F7FB] min-h-[403px]">
        <div className="text-center px-4 py-7.5">
            ...
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_, index) => (
                <Image
                  ...
                  width={14}
                  height={14}
                  className={index < Math.round(item.avgRating || 0) ? "" : "opacity-30"}
                />
              ))}
            </div>
          ...
          <h3 className="font-medium text-dark ease-out duration-200 hover:text-blue mb-1.5">
            <Link href={`/products?id=${item.id}`}> {item.name} </Link>
          </h3>
        ...
        <div className="flex justify-center items-center">
          <Image 
            src={item.thumbnail || item.productVariants?.[0]?.image || "/images/products/product-1-bg-1.png"} 
            alt={item.name} 
            width={280} 
            height={280} 
          />
