...
const Signup = () => {
  ...
  const validateAll = (u: { username: string; email: string; password: string; confirmPassword: string; }) => {
    const e: Record<string, string> = {};

    if (!u.username) e.username = "Username cannot be blank";

    if (!u.email) e.email = "Email cannot be blank";
    else if (!/^\S+@\S+\.\S+$/.test(u.email)) e.email = "Email is not valid";

    if (!u.password) e.password = "Password cannot be blank";
    else {
      if (u.password.length < 6) e.password = "Password must be at least 6 characters";
    }

    if (!u.confirmPassword) e.confirmPassword = "Confirm Password cannot be blank";
    else if (u.password && u.confirmPassword && u.password !== u.confirmPassword)
      e.confirmPassword = "Password and Confirm Password do not match";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fieldErrors = validateAll(user);
    setErrors(fieldErrors);

    if (Object.keys(fieldErrors).length > 0) {
      // Lấy lỗi đầu tiên để hiển thị lên toast
    ...
    try {
      ...
      let errorMessage = 'Lỗi kết nối tới server';
                ...
                <div className="mb-5">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setUser((prevUser) => ({ ...prevUser, password: newPass }));
                      if (showErrors) {
                        const next = { ...errors };
                        delete next.password;
                        // if confirm exists, revalidate match
                        if (user.confirmPassword && newPass !== user.confirmPassword) next.confirmPassword = 'Password và Confirm Password không khớp';
                ...
                <div className="mb-5.5">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setUser((prevUser) => ({ ...prevUser, confirmPassword: newConfirm }));
                      if (showErrors) {
                        const next = { ...errors };
                        if (!newConfirm) next.confirmPassword = 'Confirm Password không được để trống';
                        else if (user.password !== newConfirm) next.confirmPassword = 'Password và Confirm Password không khớp';
                        else delete next.confirmPassword;
                        setErrors(next);
                      }
                    }}
