...
export const addItemToWishlist = createAsyncThunk<
  ...
  number,
  { state: RootState; rejectValue: string }
>("wishlist/addItem", async (productVariantId, { getState, rejectWithValue }) => {
  try {
    const userId = getState().auth.user?.id;
    if (!userId) {
      return rejectWithValue("Người dùng chưa đăng nhập");
    }
    const data = await addWishlist(userId, productVariantId);
    return data;
  } catch (error: any) {
...
export const removeItemFromWishlist = createAsyncThunk<
  ...
  number,
  { state: RootState; rejectValue: string }
>("wishlist/removeItem", async (productVariantId, { getState, rejectWithValue }) => {
  try {
    const userId = getState().auth.user?.id;
    if (!userId) {
      return rejectWithValue("Người dùng chưa đăng nhập");
    }
    await deleteWishlist(userId, productVariantId);
